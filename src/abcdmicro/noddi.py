...
from abcdmicro.util import PathLike, update_volume_metadata, create_estimate_volume_resource
...
class Noddi:
    ...
    def load(self) -> Noddi:
        ...
        return Noddi(
            ...
            directions=self.directions.load()
        )
    ...
    def save(self, path: PathLike) -> Noddi:
        ...
        directions_path = str(path).replace(".nii", "_directions.nii").replace(".nii.gz", "_directions.nii.gz")
        return Noddi(
    ...
    @staticmethod
    def estimate_from_dwi(dwi: Dwi, mask: VolumeResource | None = None, dPar: float = 1.7e-3) -> Noddi:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            amico.util.fsl2scheme(dwi.bval.path,
                                  dwi.bvec.path,
        ...
        # AMICO also has options for RMSE, NRMSE - how the predicted signal differes from DWI signal. 
